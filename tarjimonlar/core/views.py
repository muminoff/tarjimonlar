...
def members_page(request):
    ...
    top_liked_posters = Post.objects.annotate(creator_times=Count('creator')).order_by('-likes', distinct=True)[:10]
    top_liked_commentors = Comment.objects.annotate(creator_times=Count('creator')).order_by('-likes', distinct=True)[:10]
    context = {
